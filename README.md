...
| GET    | `/config`           | Dashboard setup info: `hooks_repo`, `hook_base_url`, `reload_secret_configured` (a flag — the secret's VALUE is never returned; the setup UI shows a placeholder and the operator pastes the value from the server env), plus `run_retention` — the persisted run-history window as a compact duration (e.g. `48h`), i.e. how far back `/runs?before=` paging can ever reach — present only when the run store is configured. |
