...
- **Dashboard**: HTML view at `/` on the admin port showing the
  ...
  reconnects. The chart is **positively recovering**: each heartbeat
  carries the server's current active run-id set and the view reconciles
  to it — spans the server no longer knows drop, active runs it knows and
  the page never saw appear — so a missed update can never show fiction
...
| GET    | `/config`           | Dashboard setup info: `hooks_repo`, `hook_base_url`, `stats_url`, `reload_secret` (each only when set), plus `run_retention` — the persisted run-history window as a compact duration (e.g. `48h`), i.e. how far back `/runs?before=` paging can ever reach — present only when the run store is configured. |
...
| `WEBHOOK_RUNNER_STATS_URL`        | (none)                       | Base URL of a [simple-stats-api](https://github.com/wow-look-at-my/simple-stats-api) instance (e.g. `http://stats:3847`). The dashboard graphs host and per-container cpu/ram/disk/net/gpu from it. Unset = the dashboard says no stats source is configured. |
