...
// runContext returns the context bounding container processing: the parent with the hook's absolute timeout applied, or — when the hook sets no timeout () — a plain cancellable child with NO deadline, so an.
...
// runRef names a run for the activity feed: the id, plus the friendly title when is set — `abc… (wow-look-at-my/go-toolchain#)` — so the.
...
func (r *Runner) killContainer(name string) {
	...
	if out, err := cmd.CombinedOutput(); err != nil {
		// docker kill exits non- if the container is already
