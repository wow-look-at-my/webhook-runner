...
// queueSignal returns an onQueue callback that signals ch exactly once, on the waiter's first notification — its registration, which runs.
...
func TestLimitRaiseAdmitsQueuedWaiters(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Raise the limit: EVERY queued waiter must acquire promptly, without any holder releasing.
			tc.raise(t, m)
...
func TestLimitLowerMigratesQueuedWaiters(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Exactly two migrate into the fresh limit-2 channel; the third keeps contending at the lowered limit.
			...
			// An OLD holder releasing frees nothing at the new limit — its token lives in the retired channel.
			holderRels[0]()
			...
			// Drain everything; the new semaphore gates at exactly 2 — no token lost or double-counted.
			relW2()
			relW3()
...
func TestCancelAfterWaiterMigration(t *testing.T) {
	...
	// Raise to 2: exactly two waiters fit the fresh semaphore; the third migrates and re-blocks on the new (now full) channel.
	require.NoError(t, m.SetLimitOverride("g", 2))
...
func TestUpdateRemovingGroupFailsQueuedWaiters(t *testing.T) {
	...
	// The wait line is gone; the removed group's holder stays listed until it releases (name-keyed bookkeeping), and its release stays safe.
...
func TestQueueBookkeepingCoherentAcrossLimitRaise(t *testing.T) {
	...
	// Active counts the CURRENT semaphore's tokens only — the two migrated waiters; the pre-raise holders drain into the retired channel.
