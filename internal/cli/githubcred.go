...
// newGitHubStatusClient builds the runner's own GitHub client from whichever credential the deployment configured. sources, in precedence order: . WEBHOOK_RUNNER_GITHUB_TOKEN -- the credential pasted straight into the environment. Explicit beats derived, so it wins when both are set. . WEBHOOK_RUNNER_SECRET_SERVER_TOKEN -- an sst_ machine token for secret-server, from which the client reads PRIVATE_ORG_REPO_READ (or WEBHOOK_RUNNER_GITHUB_TOKEN_SECRET) per call. The exists because the is provisioning by hand, and hand provisioning silently does not happen: a deployment that never received the variable answers every reconciliation poll with "no GitHub token configured ... serving <old sha>" and holds deploys until a human notices. The org already keeps that credential in secret-server; this lets the runner go get it. A misconfigured machine token is a STARTUP ERROR, not a warning.
