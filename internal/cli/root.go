...
Configuration via environment:
  ...
  WEBHOOK_RUNNER_EXPECT_DATA_ROOT     path docker's store must sit under; a mismatch is a loud needs-attention entry (see deploy/pool-storage/)
...
func init() {
	...
	rootCmd.Flags().StringVar(&serveOptsRoot.scratchDir, "scratch-dir", "", "host directory absorbing hook.json 'scratch' paths, off docker's data-root (env WEBHOOK_RUNNER_SCRATCH_DIR)")
