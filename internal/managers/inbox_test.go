...
func TestInboxWatchdogArming(t *testing.T) {
	...
	go func() {
		// A generous ceiling, not a race window: the push below lands the instant we observe the park, so this wait never actually elapses -- it only has.
		_, _, _ = ib.Next(context.Background(), "i", 2*time.Second)
		close(done)
	}()
	// Synchronize on the goroutine actually being PARKED, not merely on the disarm.
	...
	// A push while PARKED delivers immediately: no wedge-guard arm, one checkout arm.
	ib.PushDelivery(nil, []byte(`{}`))
	<-done
