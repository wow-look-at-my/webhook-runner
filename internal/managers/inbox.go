...
// ErrNotSession: the presented identity is not the manager's CURRENT instance — a stale token from a previous (dead) instance, or a foreign.
...
// Delivered is a pushed delivery's completion handle: Done closes when the manager finished processing the event (its NEXT /inbox/next.
...
// Completed reports whether the manager actually finished processing the event (vs dropped/abandoned).
...
// nextWakeInterval bounds how late Next notices a deadline or a cancelled context; a push broadcasts, so it is not event latency.
...
type Inbox struct {
	...
	// onChange is the admin-surface seam (Supervisor.SetOnChange): depth, the last-delivered/last-tick stamps, and the instance binding.
	onChange func()
}

// SetOnChange registers fn to run after every mutation the admin surface would show (queue depth, delivery/tick stamps, instance binding).
...
// BindInstance attaches the current instance: its identity (the token /inbox/next must present) and the watchdog hooks.
...
func (ib *Inbox) push(e entry) {
	...
	// The wedge guard: an event queued while the manager is neither parked in Next nor mid-event means it is not consuming — arm the watchdog so a.
	arm := ib.arm
...
func (ib *Inbox) Next(ctx context.Context, instanceID string, wait time.Duration) (Event, bool, error) {
	...
	// Count the consumer present for the WHOLE call, before checkedOut is cleared below.
	ib.parked++
	var done *entry
	var disarm func()
	...
	// cond.Wait has no timeout; a ticking broadcaster keeps the loop honest against the deadline and the context.
