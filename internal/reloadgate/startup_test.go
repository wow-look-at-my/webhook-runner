...
func TestStartupTotalGitFailureKeepsLastGoodRecord(t *testing.T) {
	// Every git op fails (dead remote, broken clone): startup must degrade to serving whatever the tree holds — no apply, loud — and must NOT.
	statePath := filepath.Join(t.TempDir(), "reload-gate.json")
	seedState(t, statePath, gateState{ServingSHA: "A", Verified: true})
	repo := &fakeRepo{
	...
	// The no-persist-on-degrade guarantee: the on-disk record still names the original last-good sha.
	st := readState(t, f.statePath)
...
func TestStartupFreshThenGreenVerifies(t *testing.T) {
	...
	// The first green for the serving tree verifies it in place: no reset, no apply, entry resolved.
