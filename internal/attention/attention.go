...
const (
	// SourceLoad: a hook (or the hooks tree / concurrency.json) failed to load or validate and was DROPPED.
	SourceLoad = "load"
	// SourceZeroHooks: the loader found NO hooks at all (hooks.ZeroHooksError — the fleet-offline guard).
	SourceZeroHooks = "zero-hooks"
	// SourceSecrets: the serve-time static probe — unresolvable ${NAME} api_key/env references and sops decrypt failures on LOADED hooks.
	SourceSecrets = "secrets"
	// SourceServer: boot-scoped server-topology verdicts (the containerized TMPDIR hazard). Cannot clear without a restart.
	SourceServer = "server"
	// SourceEvent: entries derived from recognized activity-event kinds via registered rules.
	SourceEvent = "event"
	// SourceReload: the hooks-repo reload CI gate (internal/reloadgate) — a newer commit held awaiting its gating status, or the serving tree.
	SourceReload = "reload"
	// SourceTreeRefused: the whole hooks tree failed to apply.
	SourceTreeRefused = "tree-refused"
	// SourceManager: a manager that should be running has no live instance (start failing, crash-looping, image unbuildable).
	SourceManager = "manager"
	// SourceSchedule: a hook declaring a `schedule` interval has not SUCCEEDED within its staleness threshold (CheckStaleSchedules).
	SourceSchedule = "schedule"
	// SourceGitHubStatus: a loaded entity declares `github_status` while the runner holds no GitHub credential, so every status it should.
	SourceGitHubStatus = "github-status"
)

// Entry is one active problem. Identity is (Source, Hook, Key); Message is display text and may update in place without resetting Since.
type Entry struct {
...
// Resolution names entries an event rule clears: every entry under (Source, Hook) whose Key starts with KeyPrefix ("" matches all keys for.
...
// RuleFunc maps one recorded activity event (its hook field and message) onto attention mutations: entries to report (added, or refreshed.
...
type Aggregator struct {
	...
	// onChange, when set, is invoked after any mutation that actually changed the set (added, removed, or reworded an entry) — synchronously on.
	onChange func()
}
...
// SetOnChange registers fn to run after every real mutation. Set once at wiring time, before concurrent use.
...
// RegisterEventRule registers a rule for one activity-event kind. Multiple rules per kind run in registration order.
...
// notifyLocked fires onChange when a mutation actually happened; caller holds a.mu (the callback contract says trivial, so invoking under.
