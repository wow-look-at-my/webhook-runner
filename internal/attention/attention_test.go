...
func TestSinceStableAcrossRederivationsResetsOnRecur(t *testing.T) {
	...
	// The failure reason morphs while the hook stays broken: message updates in place, Since still marks when it FIRST broke.
	tick(5 * time.Minute)
...
func TestObserveEventStandardRules(t *testing.T) {
	...
	// The reserved hook-emitted class: one entry per distinct message, cleared all at once by the paired healthy signal from the same hook.
	...
	// Hook-less events of recognized kinds are dropped (nothing to pin the problem on).
