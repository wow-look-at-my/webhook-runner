...
type HookDetail struct {
	...
	// Config is the hook's own hook.json, passed through a key whitelist (see hookconfig.go): every non-secret key exactly as authored, with.
	Config HookConfig         `json:"config"`
	Image  runner.ImageStatus `json:"image"`
	// Disabled is the operator kill switch (operational state, not hook.json config — which is why it sits beside Info, not in it): true means.
	Disabled bool `json:"disabled"`
	// KV is the hook's state-store namespace summary (key count + bytes — this endpoint stays value-free like the bare /kv stats; keys and values.
	KV *kv.NamespaceStat `json:"kv,omitempty"`
	// Stats cover the live tracker window merged with the persisted run history when a run store is configured (Stats.Retention names the.
