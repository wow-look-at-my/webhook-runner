...
func newQueueServer(t *testing.T) (*Server, *kv.Store) {
	t.Helper()
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	dir := t.TempDir()
	store, err := kv.New(kv.Config{Dir: filepath.Join(dir, "kv")}, []byte("server-test-secret"), logger)
	require.NoError(t, err)
	qs, err := queue.New(queue.Config{Dir: filepath.Join(dir, "queues")}, logger)
	require.NoError(t, err)
	return New(Options{Logger: logger, KV: store, Queues: qs}), store
}
...
// cursor.
func TestStateQueuePushTakeAcrossRuns(t *testing.T) {
	s, store := newQueueServer(t)
	runA := store.Token("h", "run-a")
	runB := store.Token("h", "run-b")

	rr := stateReq(t, s, "POST", "/queue/backlog/push", runA, strings.NewReader(`{"items":["o/r#1","o/r#2","o/r#3"]}`))
	...
	// run that filled it.
	rr = stateReq(t, s, "POST", "/queue/backlog/take", runB, strings.NewReader(`{"count":2}`))
	require.Equal(t, http.StatusOK, rr.Code)
	var take struct {
		Items []string
		Depth int
	...
	rr = stateReq(t, s, "POST", "/queue/backlog/push", runB, strings.NewReader(`{"items":["o/r#1","o/r#2","o/r#3"]}`))
	require.Equal(t, http.StatusOK, rr.Code)
	require.NoError(t, json.Unmarshal(rr.Body.Bytes(), &push))
	...
	rr = stateReq(t, s, "GET", "/queue/backlog", runA, nil)
	require.Equal(t, http.StatusOK, rr.Code)
	var stat queue.Stat
	require.NoError(t, json.Unmarshal(rr.Body.Bytes(), &stat))
	assert.Equal(t, queue.Stat{Name: "backlog", Depth: 3}, stat)
	assert.NotContains(t, rr.Body.String(), "o/r#", "a depth read never exposes the contents")

	rr = stateReq(t, s, "GET", "/queues", runA, nil)
	require.Equal(t, http.StatusOK, rr.Code)
	var stats []queue.Stat
	require.NoError(t, json.Unmarshal(rr.Body.Bytes(), &stats))
	assert.Equal(t, []queue.Stat{{Name: "backlog", Depth: 3}}, stats)
}

// Namespaces come from the token, never the URL — a hook cannot reach another
// hook's backlog.
func TestStateQueueNamespaceIsolation(t *testing.T) {
	s, store := newQueueServer(t)

	require.Equal(t, http.StatusOK,
		stateReq(t, s, "POST", "/queue/q/push", store.Token("h1", "run-a"), strings.NewReader(`{"items":["x"]}`)).Code)

	rr := stateReq(t, s, "POST", "/queue/q/take", store.Token("h2", "run-b"), strings.NewReader(`{"count":5}`))
	require.Equal(t, http.StatusOK, rr.Code)
	var take struct{ Items []string }
	require.NoError(t, json.Unmarshal(rr.Body.Bytes(), &take))
	assert.Empty(t, take.Items, "h2 sees its own empty queue, not h1's work")

	require.Equal(t, http.StatusUnauthorized, stateReq(t, s, "POST", "/queue/q/take", "", nil).Code)
}

func TestStateQueueValidation(t *testing.T) {
	s, store := newQueueServer(t)
	tok := store.Token("h", "run-a")

	for _, body := range []string{`{"items":[]}`, `{}`, `not json`, `{"items":[""]}`} {
		rr := stateReq(t, s, "POST", "/queue/q/push", tok, strings.NewReader(body))
		assert.Equalf(t, http.StatusBadRequest, rr.Code, "push body=%q", body)
	}
	for _, body := range []string{`{"count":0}`, `{"count":-1}`, `{"count":1001}`, `nope`} {
		rr := stateReq(t, s, "POST", "/queue/q/take", tok, strings.NewReader(body))
		assert.Equalf(t, http.StatusBadRequest, rr.Code, "take body=%q", body)
	}
	// An empty take body is the documented default of one item.
	require.Equal(t, http.StatusOK, stateReq(t, s, "POST", "/queue/q/push", tok, strings.NewReader(`{"items":["a","b"]}`)).Code)
	rr := stateReq(t, s, "POST", "/queue/q/take", tok, nil)
	require.Equal(t, http.StatusOK, rr.Code)
	var take struct{ Items []string }
	require.NoError(t, json.Unmarshal(rr.Body.Bytes(), &take))
	...
	assert.Equal(t, http.StatusBadRequest, stateReq(t, s, "GET", "/queue/Bad_Name", tok, nil).Code)
}

// Without a queue store the routes are honestly unavailable rather than
// silently pretending an empty backlog.
func TestStateQueueUnconfigured(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	...
	tok := store.Token("h", "run-a")
	assert.Equal(t, http.StatusServiceUnavailable, stateReq(t, s, "POST", "/queue/q/push", tok, strings.NewReader(`{"items":["a"]}`)).Code)
	assert.Equal(t, http.StatusServiceUnavailable, stateReq(t, s, "POST", "/queue/q/take", tok, nil).Code)
	assert.Equal(t, http.StatusServiceUnavailable, stateReq(t, s, "GET", "/queue/q", tok, nil).Code)
	assert.Equal(t, http.StatusServiceUnavailable, stateReq(t, s, "GET", "/queues", tok, nil).Code)
}
