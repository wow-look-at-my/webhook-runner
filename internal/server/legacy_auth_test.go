...
func TestVerifyLegacyHMAC(t *testing.T) {
	...
	cases := []struct {
		...
		{"wrong-mac", "sha256=" + reverseStr(want), false},
		{"wrong-secret", "sha256=" + hmacHex([]byte("other"), body), false},
		{"wrong-body", "sha256=" + hmacHex([]byte(secret), []byte("other")), false},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := verifyLegacyHMAC(body, tc.header, secret)
