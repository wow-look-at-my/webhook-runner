...
- **`seccomp.userns` lifts TWO gates, because one is a no-op.** The opt-in means "this container may build a user namespace and mount inside it". Seccomp gates the syscalls; AppArmor's `docker-default` denies `mount` independently, so passing only the profile leaves bwrap creating its namespace and dying at `Failed to make / slave`, which dats reports as "no usable sandbox backend". `seccompArgs` therefore emits `apparmor=unconfined` alongside the profile. That adds no capability — `/proc` stays masked (`systempaths` is banned) and no device appears — and the dind fleet already had it, implied by `--privileged`. Measured on GitHub's AppArmor-enforcing runners, where the two fleets ran the same suite in one job and only the privileged one passed.
