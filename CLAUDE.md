...
- **`$schema` is required.** Every `hook.json` must declare a `$schema`
  ...
  push (`.github/workflows/schemas.yml`; never GitHub Pages, which rides the
  org's exhausted Actions artifact quota). Canonical URL:
  ...
  `https://wow-look-at-my.github.io/webhook-runner/` URLs still serve frozen
  content and stay valid in deployed hook.jsons.
...
installed and measured working. The measurements, and why granting the slim
fleet `seccomp=unconfined` + `CAP_SYS_ADMIN` is not the answer, are in the
webhooks repo's `src/hooks/gha-runner/CLAUDE.md`.
Moving either job back to `CI_RUNNER` fails
...
after every build: it stages throwaway binary copies under
`$GO_TOOLCHAIN_DATS_BUILD_DIR` and does NOT put them on PATH, so a bare
`webhook-runner` in a `cmd` exits 127 there. The `:-build` fallback keeps standalone runs working from the
...
- **Retire a manifest field by expand/contract, never in one step.** Two independently-deployed artifacts cannot change a shared contract atomically: a runner that rejects the field cannot serve a fleet declaring it, and a fleet on the replacement cannot be served by a runner that predates it. Either direction leaves something unservable, and no rollback reaches it — the tree did not move, the binary did. So accept BOTH for one release, with the old field still doing exactly what it always did (a deprecation that quietly stops working is worse than a flag day), and name every remaining use in the log, the activity feed, `validate` output and the needs-attention surface. That list is the migration's to-do list; the removal merges when it is empty. `env` is the worked example, and is now an unknown field: a load error naming it.
- **The two manifest schemas share ONE base — never hand-copy a property between them.** Hand-maintained copies drift silently: a lost duration `pattern` validates `"banana"`, a lost default changes what an omitted key means. Shared CONSTRAINTS live once in `schema/src/common.json` and reach each published document as one `$defs.common` block it `allOf`-`$ref`s; prose stays per-entity (a manager's concurrency slot is held for the instance's whole lifetime, its `run_title` placeholders always resolve empty), so each overlay supplies its own description and a shared key with none is a generate error. `go test ./schema -update` regenerates.
...
- **The fleet is served ALL-OR-NOTHING, and that is what makes a contract change safe.** A load in which ANY entity fails applies NOTHING (`internal/cli.buildLoadAndApply`). Applying the partial set fails OPEN in exactly the case that produces fleet-wide load errors — a binary and a tree that disagree about a manifest field — so every entity using it stops serving silently, under a "hooks reloaded" line, with no rollback to reach for. Both deploy directions are closed instead. **Binary moved** → the startup load errors and serve exits non-zero naming the entities, so a bad image is a FAILED, VISIBLE deploy and the previous container keeps serving. **Tree moved** → the gate resets the working tree to the commit that was serving, re-applies THAT, keeps its old serving record, and never persists the refused sha (`internal/reloadgate.applyOrRollbackLocked`) — the deploy is HELD, not half-applied. Being strict is only safe because a load error cannot reach a gated deploy by the normal path: the reload gate requires the hooks repo's CI green, and that CI runs `validate` through this same loader. Pinned by `internal/cli/loadapply_refuse_test.go` and `internal/reloadgate/refuse_git_test.go`.
- **`fleet-compat` reports the deploy ORDER; it is not a safety gate — ci.yml.** It validates this binary against webhooks **master** (the deployed tree) AND the webhooks branch matching this branch's name (the tree this change is paired with — the same convention webhooks CI uses in reverse). It FAILS only when nothing the change ships with can be served, i.e. there is no order in which it becomes deployable. When master alone fails, that is a `DEPLOY ORDER` notice, because the all-or-nothing rule above makes the wrong order survivable: the worst case is a failed rollout or a held tree, both loud, neither destructive. The runtime owns the safety; this job owns the telling. Never make a coordinated pair red on purpose to enforce a merge order.
- **New hook.json fields are deploy-first.** `Parse` uses `DisallowUnknownFields`, so an older binary REJECTS a hook using a newer field. Deploy webhook-runner first; if you don't, the all-or-nothing rule above turns it into a held tree (the gate rolls back and keeps serving), not a fleet missing entities.
...
- **An image tag is a content hash, so anything derived from it is cacheable.** `imageCommand`'s `docker inspect` is memoized per (tag, command), which keeps a CLI + daemon round trip off the path between slot acquisition and container launch. Never cache an inspect FAILURE: that is a daemon condition, not a property of the tag.
...
- [docs/internals/runner-isolation.md](docs/internals/runner-isolation.md) -- keeping a container off the host while bwrap still works: the one-line primitives, the userns-remap interlock that makes an unmasked /proc safe, why --privileged is banned outright, and the per-job VM boundary none of it provides.
