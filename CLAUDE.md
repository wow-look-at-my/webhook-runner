...
- The dashboard timeline splits in two: the **`<timeline-view>` component
  ...
  banner — never hand-edit it, edit ts/ and regenerate). Rebuild it with
  `go generate ./internal/server/dashboard/` and commit the bundle with the
  source; CI runs the same directive and fails on a dirty tree, so a stale
  bundle is red rather than silent — docs/timeline-bundle-generation.md.
