...
- The hooks-repo reload CI gate (`internal/reloadgate`) moves the tree
  ...
  gating status the gate accepts is written to a bounded, TTL'd
  sha->state map in the same state file (`verdicts`, additive/omitempty),
  INCLUDING greens the ordering rule then refuses — a delivered verdict is
  a verified fact about that sha, and discarding it is what left the poll
  ...
  every apply, so a record is an input to that rule, not a bypass. Repeat ticks over an unchanged verdict are quiet. The poll makes
