...
- [docs/internals/runner-isolation.md](docs/internals/runner-isolation.md) -- keeping a container off the host: the two one-line primitives, why docker's read-only /proc bind is the whole protection, the two flags that remove it (both banned, mechanically), userns-remap as depth, and the per-job VM boundary none of it provides.
