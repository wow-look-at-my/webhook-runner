...
schema/                    JSON schemas for hook.json + manager.json + concurrency.json — published to buildhost sites (.github/workflows/schemas.yml) AND go:embed'd (embed.go) so the loader enforces the same contract at runtime. hook.schema.json + manager.schema.json are GENERATED from src/ (src/common.json holds the 24 shared property constraints ONCE; each overlay adds its own properties and the per-entity prose) — regenerate with `WEBHOOK_RUNNER_UPDATE_SCHEMAS=1 go-toolchain` (the `-update` flag still works, but go-toolchain cannot pass a test flag through and bare `go test` is blocked); a drifted checkout fails TestGeneratedSchemasMatchSources
...
- **`seccomp.userns` reaches only the syscall gate, and an AppArmor host has a second one docker cannot open.** `docker-default` grants the user namespace and denies `mount`, so bwrap dies at `Failed to make / slave`; `apparmor=unconfined` trades that for Ubuntu 24.04's unprivileged-userns restriction, so bwrap dies EARLIER, at `setting up uid map`. Both measured on GitHub's runners, one commit apart, while the privileged dind image passed the same suite — so `--privileged` is not evidence that an unprivileged container can sandbox. Only a profile loaded on the HOST lifts both gates. The fleet's host does not enforce AppArmor, which is why the slim fleet sandboxes there and cannot on a GitHub-hosted runner.
...
- **A shared BASE image is a directory under `src/base/`, named by the entity's `base` field.** Two entities that want the same foundation cannot share a Dockerfile — `FROM` names an image, never another file — and a base built outside the hashed tree would leave consumers on stale images. So the runner builds `src/base/<name>/Dockerfile` first (context `src/`, tagged `whr-base/<name>:<hash of that directory>`) and hands the tag to each consumer's build as the `BASE_IMAGE` build arg, which its Dockerfile reads as `ARG BASE_IMAGE` then `FROM ${BASE_IMAGE}`. Entities naming the same base share the built image. Invalidation needs no new rule: `src/base` is a shared dir, so `ContentHash` already folds it into every entity's own tag and a base edit rebuilds its consumers. A name is a single path segment and must resolve to a Dockerfile, checked at LOAD (`internal/hooks/imagebuild.go`) — an unresolvable base DROPS the entity rather than failing a build once a delivery arrives.
...
- [docs/internals/resource-graphs.md](docs/internals/resource-graphs.md) -- the title-bar host cpu/ram/disk/net/gpu graphs (`ts/stats.ts`). A simple-stats-api instance feeds them (`WEBHOOK_RUNNER_STATS_URL`). Nothing is measured here.
